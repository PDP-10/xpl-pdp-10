...
Version 3.0        November, 1975.
   ...
   */
   ...
   declare VERSION literally '''4.0w1''';
   ...
   */
   ...
   /* alphabet holds a list of legal characters for <identifier> */
   declare alphabet character initial(
      'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_$@#'),
      lowchar(255) bit(8);      /* Map uppercase characters to lowercase */
 
   /* number_value contains the numeric value of the last constant scanned,
   */
   declare number_value bit(36), jbase fixed, base fixed;
   ...
   */
   ...
   declare UNITS literally '5';        /* The number of I/O units */
   declare filename(UNITS) character;  /* I/O File names */
   declare unit(UNITS) fixed;          /* Holds Unit numbers for I/O */
   declare INPUTFILE literally '0';    /* XPL source file */
   ...
   declare code_buffer (BUFFERSIZE) bit(36);   /*code (high) buffer */
   declare data_buffer (BUFFERSIZE) bit(36);   /* data (low) buffer */
   declare label_buffer (BUFFERSIZE) bit(36);  /* labels defined buffer */
   declare code_rel(3) bit(36),         /* binary code buffer (see code) */
           code_pp(3) bit(36),
           code_rbits(3) bit(36);
   ...
   declare pword bit(36);                /* part-word acc. for bytes*/
   ...
   declare DESCLIMIT literally '2048', /* number of string descriptors */
...
'');
   ...
   declare (sysmark, procmark, ndecsy, maxndecsy, parct) fixed;
   ...
   declare typename (14) character initial ('', 'label    ', '', '', '', '',
           'character', 'fixed    ', 'bit (9)  ' , '', '', 'special  ',
           'procedure', 'procedure', 'character procedure');
...
pdp_date:
   procedure fixed;
        declare days(11) fixed
                initial(0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335);
        declare (year, day, m, d) fixed;
        ...
        return (year - 1964) * (31 * 12) + m * 31 + day - days(m);
   end pdp_date;
 
shift_left:
   procedure(v, n) bit(36);
      /* Simulate a 36 bit word */
      declare v bit(36), n fixed;

      return shl(v, n) & "(3)777 777 777 777";
   end shift_left;
...
shift_right:
   procedure(v, n) bit(36);
      ...
      v = v & "(3)777 777 777 777";
      return shr(v, n);
   end shift_right;

declare bin(15) bit(8);         /* 36 -> 32 bit conversion buffer */
declare binflip fixed;          /* 0 -> first word, 1 -> second word */
...
write_file:
    procedure(dat);
        ...
        /* Dump the data in Octal */
        if control(byte('O')) then do;
                call xprintf("(c)       %012lo\n", dat & "f ffff ffff");
        end;
	if binflip = 0 then do;
                dat = shl(dat, 4);
                i = 4;
                do while i >= 0;
                        bin(i) = dat;
		...
		bin(4) = bin(4) | (dat & 15);
                output(unit(RELFILE)) = build_descriptor(9, addr(bin));
                binflip = 0;
        end;
    end write_file;
...
output_one:
   procedure(text);
        /* Simulate line printer vertical format control */
        declare text character;

        if byte(text) = byte('1') then output = '';
        if byte(text) = byte('0') then output = ' ';
...
printline:
   ...
   if line_count > PAGE_MAX then
      do;
         ...
         call output_one(title || page_count);
         output = subtitle;
         output = ' ';
         line_count = 0;
      end;
   ...
   */
      ...
      if control(byte('L')) = 0 then
         do;
            ...
            call printline (substr (x70, 0, i) || card_count || x2 || buffer,-1);
         end;
...
convert_lower:
      procedure(s) character;
         declare (s, t) character, i fixed;
         t = '';
         do i = 0 to length(s) - 1;
            t = t || substr(alphabet, lowchar(byte(s, i)), 1);
         end;
         return t;
   end convert_lower;
...
build_bcd:
      ...
      byte(bcd, length(bcd) - 1) = c;  /* insert new byte */
   end build_bcd;
      ...
      do case chartype(byte(text));
         ...
         do;      /*  bit quote("):  bit string  */
            ...
            do while ch ~= byte ('"');
               ...
               if ch >= byte('A') & ch <= byte('F') then s2 = 10 - byte('A') + ch;
               ...
               if token = string then
                  do while s1 - jbase >= 9;
                     ...
                     s1 = s1 - 9;
                     call build_bcd(shift_right(number_value, s1 - jbase));
                  end;
               number_value = shift_left(number_value, jbase) + s2;
               call bchar;
            end;     /* of do while ch...  */
            cp = cp + 1;
            if token = string then
               if length(bcd) >= 255 then call error (lbitm,0);
               else call build_bcd(shift_left(number_value, 9 - s1));
             return;
         end;
         /*  case 4  */
...
flush_data_buffer:
   procedure;
      ...
      if (dptr+dctr) > 1 then
         do;
            j = (dptr/19)*18 + dctr -1;
            call write_file(CODE_TYPE + j);
            i = dptr+dctr-1;
            do j = 0 to i;
               call write_file(data_buffer(j));
               end;
         end;
      dptr = 0;
      dctr = 1;
   ...
   procedure;
      ...
      if (rptr+rctr) > 1 then
         do;
            i = (rptr/19)*18 + rctr -1;
            j = rptr+rctr-1;
            call write_file(CODE_TYPE + i);
            do i = 0 to j;
               call write_file(code_buffer(i));
               end;
         end;
      rptr = 0;
      rctr = 1;
...
radix50:
   ...
   k = 0;
   do i = 0 to length(symbol) - 1;
        if byte(symbol, i) = byte('/') then k = i + 1;
   end;
   symbol = substr(symbol, k);
...
output_codeword:
   ...
   if code_full(code_tail) then
      do;
         if control(byte('A')) then output(unit(CODEFILE)) = code(code_tail);
         if rctr+rptr = 1 then
            do;
         ...
         code_buffer(rptr) = shift_left(code_rbits(code_tail), 36 - rctr * 2) | code_buffer(rptr);
...
flush_labels:
   procedure;
      ...
      call write_file(INTREQ_TYPE + label_count);
      do i = 0 to label_count;
         call write_file(label_buffer(i));
         end;
      label_count = 0;
      label_buffer(0) = 0;
   end flush_labels;
...
flush_datacard:procedure;
      if control(byte('A')) | control(byte('B')) then
         do;
            ...
            if control(byte('A')) then output(unit(DATAFILE)) = datacard;
...
emitbyte:
   procedure (c);
      ...
      pword = pword + shift_left(c & "(3)777", 9 * (3 - dpoffset));
      dpoffset = dpoffset + 1;
      if dpoffset = 4 then call flush_datacard;
end emitbyte;

declare ctab (100) bit(36), cadd (100) fixed, nc fixed;
...
emitconstant:
   procedure (c);
      ...
      if (c & "(3)400 000 000 000") = 0 then c = c & "(3)777 777 777 777";
      else c = c | (-"(3)400 000 000 000");
...
outputlabel:
   ...
   label_buffer(0) = shift_left(3, 36 - label_count * 2) | label_buffer(0);
...
emitlabel:procedure(l,r);
      ...
      do i = 1 to for_count;
         if j = for_label(i) then
            do;
               j = shift_left(for_ref(i) + "(3)400000", 18);
...
emitinst:procedure (opcode,treg,indirect,operand,ireg,relocation);
      ...
      word = shift_left(opcode, 27) + shift_left(treg & "f", 23) +
             shift_left(indirect & 1, 22) + shift_left(ireg & "f", 18);
      do case relocation;
         ...
         do;
            i = shift_left(relocation, 18) + (operand & "(3)777777");
            j = refcheck(i);
            word = word + j;
            if j = 0 then rbits = 0;
                     else rbits = 1;
...
hasher:
   procedure (id) fixed;          /* calculate hash index into hash table*/
   declare id   character;
   declare l    fixed;
   l = length (id);
...
symbol_search:
   procedure(p) fixed;
      ...
      do while i ~= -1;
         idcompares = idcompares + 1;
         if char_temp = syt(i) then return i;
         i = ptr(i);
      end;
      /* A linear search of the builtin functions.  We cannot use the hash
         function on compares that ignore case.
      */
      do i = 0 to sysmark;
         if length(char_temp) = length(syt(i)) then do;
            idcompares = idcompares + 1;
            j = 0;
            do while lowchar(byte(char_temp, j)) = lowchar(byte(syt(i), j));
               j = j + 1;
               if j >= length(char_temp) then return i;
            end;
         end;
      end;
      /* not found */
            ...
            if dsp < newdsp then
               do;
                  ...
                  if i = 0 then
                     ...
                     do;
                        call emitdesc(i, dpoffset + shift_left(dp, 2));
                        do j = 0 to i - 1;
         ...
         do;  /* fetch a descriptor into an accumulator */
            ...
            if sl = 0 then call emitinst (movei,r,0,0,0,0);
               ...
               do;  /* generate descriptor and string, then pick it up */
                  ...
                  call emitdesc(sl, shift_left(dp, 2) + dpoffset);
                  do r = 0 to sl-1;
                     call emitbyte (byte(t1,r));
                  end;
               end;
         end;
...
symboldump:
   procedure;
      ...
      */
      ...
      declare seg(5) character initial ('absolute','    data',' program',
               '  string', '   label', '    data');
...
/*      call printline ('compactifications = ' || count_compact,-1); */
      ...
      subtitle = 'Constant table';
      EJECT_PAGE;
      do i = 1 to nc;
         call i_format(cadd(i), 8);
         call printline(i_string || ' ' || ctab(i), -1);
      end;
end dumpit;
...
initialize:
   procedure;
      ...
      year = date1 / 12 + 1964;
      title = '1' || filename(INPUTFILE) || '  Compiled ' || day || month ||
   ...
   do i = TERMINAL# to  VOCAB#;
      ...
      chartype (byte(' ')) = 1;
      chartype (byte('	')) = 1;    /* make the TAB character a blank */
      chartype (byte('''')) = 2;
      chartype (byte('"')) = 3;
      do i = 0 to 255;
      ...
      do i = 0 to length(alphabet) - 1;
         ...
         chartype(j) = 4;
         if i < 26 then lowchar(byte(alphabet, i)) = i + 26;
         else lowchar(byte(alphabet, i)) = i;
      end;
      do i = 0 to 9;
         j = byte('0123456789', i);
         not_letter_or_digit(j) = FALSE;
...
/*    initialize the symbol table and its hash table */
      ndecsy = 27;
      procmark = 25;  /* procmark is set to pick up compactify */
      sysmark = ndecsy;
      parct = 0;
      do i = 0 to SYTSIZE;
         ptr (i) = -1;
      ...
      call write_file(NAME_TYPE + 2);
      call write_file(0);
      call write_file(radix50(filename(INPUTFILE)));
      call write_file("(3)17000000" + 0);
      call write_file(HISEG_TYPE + 1);
      call write_file("(3)200000000000");
      call write_file("(3)400000400000");
      ...
      if control(byte('A')) then
         do;
            ...
            output(unit(DATAFILE)) = '       title ' || filename(INPUTFILE);
            output(unit(DATAFILE)) = '       twoseg 400000;';
            output(unit(DATAFILE)) = '       reloc 0;';
            output(unit(DATAFILE)) = '       radix 10;';
            output(unit(CODEFILE)) = '       reloc |o400000;';
            output(unit(DATAFILE)) = '       opdef   .init. [1b8];';
            output(unit(DATAFILE)) = '       opdef   .inpt. [2b8];';
            output(unit(DATAFILE)) = '       opdef   .outp. [3b8];';
            output(unit(DATAFILE)) = '       opdef   .exit. [4b8];';
            output(unit(DATAFILE)) = '       opdef   .fili. [6b8];';
            output(unit(DATAFILE)) = '       opdef   .filo. [7b8];';
            output(unit(DATAFILE)) = '       opdef   .name. [8b8];';
            output(unit(DATAFILE)) = 'd:';
         end;
      byteptrs = dp;
      ...
      if control(byte('A')) then
         do;
            output(unit(DATAFILE)) = '       xpllib;';
            output(unit(DATAFILE)) = '       extern xpllib;';
         end;
      dp = dp + 1;
...
/*     52   <declaration element> ::= <type declaration>             */
      if type (mp) = CHRTYPE then
         do while (dsp < newdsp & compiling = TRUE);
            call emitdesc (0,0);
         end;
      else
         do;
               ...
               do;
                  if type(mpp1) = CONSTANT then
                     do;  /* emit a complex CONSTANT */
                        call emitconstant(shift_left(fixv(mpp1), 27) - fixv(mpp1));
               ...
               do;  /* three arguments */
                  if type(mpp1) = CONSTANT then
                     do;  /* make a CONSTANT length to or in */
                        call emitconstant(shift_left(fixv(mpp1), 27));
               ...
               do; /* just substr, we'll note error on byte later */
                  if type(mpp1) ~= CONSTANT | fixv(mpp1) ~= 0 then
                     do;
                        sp = mpp1;  /* so we can use arithemit */
                       call arithemit (add,1);
                        fixv(mp) = 2; /* if it commutes, arithemit changes it */
                     end;
...
conflict: procedure (current_state) fixed;
...
compile:
   ...
   time_start = pdp_time;           /* get time(cpu) started*/
   call initialize;
   time_init = pdp_time;     /* time to initialize */
   call compilation_loop;
   time_compile = pdp_time;     /* time to compile the program*/
   ...
   if control(byte('A')) then
      output(unit(DATAFILE)) = '$' || ndesc || ':'; /* label for assembler */
   call emitdataword (dsp-1);           /* put down number of desc's */
   if control(byte('A')) then
      output(unit(DATAFILE)) = 's=.;';           /* start string segment */
   /* add the descriptors to the data segment                                 */
   do i = 0 to dsp-1;
      if control(byte('A')) then
         output(unit(DATAFILE))='       byte (9)'||descl(i)|| '(27)' ||desca(i)|| ';';
      call output_dataword(shift_left(descl(i), 27) + desca(i), dp);
      call emitlabel (i,3);
      dp = dp + 1;
   end;
   ...
   if control(byte('A')) then
      do;
         output(unit(CODEFILE)) = '       end $0;';
         /* Simulate a rewind */
         call xfclose(unit(CODEFILE));
         unit(CODEFILE) = xfopen(filename(CODEFILE), 'r');
         if unit(CODEFILE) < 0 then do;
             call error('Unable to reopen temp code file: ' || filename(CODEFILE), 1);
         end;
         /* copy code file to end of data file */
         codestring = input(unit(CODEFILE));
         do while length(codestring) > 0;
            output(unit(DATAFILE)) = codestring;
            codestring = input(unit(CODEFILE));
         end;
      end;
 
   call write_file(SYMB_TYPE + 2);      /* generate external refs */
   call write_file("(3)040000000000");
   call write_file("(3)600000000000" + radix50 ('xpllib'));
   call write_file(library);
   call write_file(START_TYPE + 1);
   call write_file("(3)200000000000");
   call write_file("(3)400000" + startloc);
   call write_file(END_TYPE + 2);
   call write_file("(3)240000000000");
   call write_file("(3)400000" + pp);
   call write_file(dp);

   /* Flush out the last four bits */
   if binflip = 1 then call write_file(0);
 
   time_finish = pdp_time;   /* time to do all but final stats */
...
basename:
   procedure(name) character;
        ...
        j = 0;
        do i = 0 to length(name) - 1;
                if byte(name, i) = byte('/') then j = i + 1;
        end;
        name = substr(name, j);
        j = length(name);
        do i = 0 to length(name) - 1;
                if byte(name, i) = byte('.') then j = i;
        end;
        if j > 0 then return substr(name, 0, j);
        return name;
   end basename;
...
open_files:
   procedure fixed;
        ...
        /* Input source */
        if length(filename(INPUTFILE)) > 0 then do;
                unit(INPUTFILE) = xfopen(filename(INPUTFILE), 'r');
                if unit(INPUTFILE) < 0 then do;
                        output = 'Unable to open source file: ' || filename(INPUTFILE);
                        return 0;
                end;
        end;
        else return 0;

        if length(filename(LIBFILE)) = 0 then filename(LIBFILE) = 'xpl.lib';
        unit(LIBFILE) = xfopen(filename(LIBFILE), 'r');
        if unit(LIBFILE) < 0 then do;
                output = 'Unable to open library file: ' || filename(LIBFILE);
                return 0;
        end;

        if length(filename(RELFILE)) = 0 then
                filename(RELFILE) = basename(filename(INPUTFILE)) || '.rel';
        unit(RELFILE) = xfopen(filename(RELFILE), 'wb');
        if unit(RELFILE) < 0 then do;
                output = 'Unable to open object file: ' || filename(RELFILE);
                return 0;
        end;

        if control(byte('A')) then do;
                if length(filename(CODEFILE)) = 0 then
                        filename(CODEFILE) = basename(filename(INPUTFILE)) || '.tmp';
                unit(CODEFILE) = xfopen(filename(CODEFILE), 'w');
                if unit(CODEFILE) < 0 then do;
                        output = 'Unable to open temp code file: ' || filename(CODEFILE);
                        return 0;
                end;
        end;

        if control(byte('A')) then do;
                if length(filename(DATAFILE)) = 0 then
                        filename(DATAFILE) = basename(filename(INPUTFILE)) || '.mac';
                unit(DATAFILE) = xfopen(filename(DATAFILE), 'w');
                if unit(DATAFILE) < 0 then do;
                        output = 'Unable to open temp data file: ' || filename(DATAFILE);
                        return 0;
                end;
        end;
...
usage:
   procedure;
        output = 'Usage:';
        output = '   ' || argv(0) || '[-ABCDEFKLMORSX] [-d datafile] [-l libfile] [-o relfile] [-t tempfile] source-file';
        output = '      -d datafile - Scratch data file (default: source.mac)';
        output = '      -l libfile - Library file (default: xpl.lib)';
        output = '      -o relfile - Relocatable object file (defailt: source.rel)';
        output = '      -t tempfile - Scratch code file (defailt: source.tmp)';
        output = '      source-file - XPL input source file name';
        output = '';
        output = '   Uppercase letters set initial values for compiler toggles';
        output = '      A - produce macro-10 source file for program';
        output = '      B - list emitted data and descriptors';
        output = '      C - list emitted constants';
        output = '      D - list macros and compiler info at end of program';
        output = '      E - list emitted code';
        output = '      F - (if L = true) list code, data, descriptor addresses';
        output = '      K - kill listing output (inhibits L being set)';
        output = '      L - produce listing with line #, current procedure, etc';
        output = '      M - list just image of input source (overrides L)';
        output = '      O - Display the binary file in octal';
        output = '      R - dump parse stack on listing on syntax errors';
        output = '      S - (if L = true) list symbol table for each proc.';
        output = '      X - do not include contents of ''xpl.lib''';
   end usage;

do i = 1 to argc - 1;
        if byte(argv(i), 0) = byte('-') then do;
                if byte(argv(i), 1) = byte('d') then do;
                        i = i + 1;
                        if i < argc then do;
                                filename(DATAFILE) = argv(i);
                                control(byte('A')) = TRUE;
                        end;
                        else output(1) = 'missing data filename';
                end;
                else
                if byte(argv(i), 1) = byte('l') then do;
                        i = i + 1;
                        if i < argc then filename(LIBFILE) = argv(i);
                        else output(1) = 'missing library filename';
                end;
                else
                if byte(argv(i), 1) = byte('o') then do;
                        i = i + 1;
                        if i < argc then filename(RELFILE) = argv(i);
                        else output(1) = 'missing object filename';
                end;
                else
                if byte(argv(i), 1) = byte('t') then do;
                        i = i + 1;
                        if i < argc then do;
                                filename(CODEFILE) = argv(i);
                                control(byte('A')) = TRUE;
                        end;
                        else output(1) = 'missing temp code filename';
                end;
                else
                ...
                filename(INPUTFILE) = argv(i);
        end;
end;

if length(filename(INPUTFILE)) = 0 then do;
        output(1) = 'Missing source file name';
        call usage;
        return 1;
end;

if open_files then do;
        call compile;
end;
